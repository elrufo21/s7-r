...
export function FrmTab1({ watch, setValue }: frmElementsProps) {
  ...
  const isReadOnly = true //watch('payment_state') === TypeStatePayment.PAYMENT || watch('state') === 'C'
  ...
  console.log("watch('payment_state')", watch('payment_state'))
    ...
    () => [
      {
        ...
        size: 120,
        minSize: 100,
        cell: ({ row }) => <span className="text-sm">{formatPlain(row.original.date)}</span>,
      },
      {
...

      {
        ...
        cell: ({ row }) => (
          <div className="flex justify-center items-center">
            {
              /**watch('payment_state') === 'PF' */ isReadOnly ? (
                <></>
              ) : (
                <button
                  type="button"
