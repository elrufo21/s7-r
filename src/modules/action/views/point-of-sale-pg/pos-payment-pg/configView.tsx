...
export function FrmMiddle({ control, errors, editConfig, watch, setValue }: frmElementsProps) {
  ...
  useEffect(() => {
    setFrmConfigControls({
      ...
      reason: { isEdit: true },
      type: { isEdit: true },
      session_name_2: { isEdit: true },
      partner_name_2: { isEdit: true },
    })
  }, [])
  useEffect(() => {
    if (!formItem) return
  ...
  return (
    <>
      {formItem?.origin === Type_pos_payment_origin.DIRECT_PAYMENT ||
      formItem?.origin === Type_pos_payment_origin.PAY_DEBT ? (
        <>
          <BaseTextControlled
            label="Motivo"
            name={'reason'}
          ...
          {formItem?.origin === Type_pos_payment_origin.PAY_DEBT && (
            <BaseTextControlled
              ...
              placeholder={''}
              editConfig={editConfig}
              navigationConfig={{ modelId: 400, recordId: formItem?.session_id_2 }}
            />
          )}
        </>
      ) : (
        <>
          <BaseTextControlled
            ...
            placeholder={''}
            editConfig={editConfig}
            navigationConfig={{ modelId: 400, recordId: formItem?.session_id }}
          />
          <BaseTextControlled
            label="Orden"
            name={'order_name'}
            ...
            placeholder={''}
            editConfig={editConfig}
            navigationConfig={{ modelId: 401, recordId: formItem?.order_id }}
          />
        </>
      )}
      ...
      <BaseTextControlled
        ...
        placeholder={''}
        editConfig={editConfig}
        navigationConfig={{ modelId: 406, recordId: formItem?.payment_method_id }}
      />
      {formItem?.origin === Type_pos_payment_origin.PAY_DEBT && (
        <BaseTextControlled
          label="Cliente"
          ...
          placeholder={''}
          editConfig={editConfig}
          navigationConfig={{ modelId: 403, recordId: formItem?.partner_id_2 }}
        />
      )}
    </>
  )
