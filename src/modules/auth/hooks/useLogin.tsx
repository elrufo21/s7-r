...
export const useLogin = (): {
  ...
  const { setUserLoginInfo, setMessageError } = useUserStore()
  const user = 'desarrollo.linksoft@hotmail.com'
  const pswrd = '12345678'
  ...
  const login = async (email: string, password: string): Promise<void> => {
    await setUserLoginInfo({ email, password })
    setLoading(true)
    setHasErrors(false)
    if (!isOnline) {
    ...
    const userData = await fnc_user_login(email, password)
    if (error || !userData.success) {
      setMessageError(userData.message || 'Error desconocido')
      setHasErrors(true)
      setLoading(false)
    } else {
      setUserSession(userData.data)
