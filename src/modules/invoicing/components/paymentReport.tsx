...
const formatAmount = (amount, currency = 'S/ ') => {
      ...
      */}
      ...
      <View style={styles.section}>
        <View style={styles.sectionHead}>
          <Text style={styles.sectionHeadTitle}>REPORTE DE PAGOS</Text>
        </View>
        ...
        <View style={styles.twoColumnContainer}>
          ...
          <View style={styles.columnLeft}>
            {Array.isArray(data?.rows) &&
              data.rows
                .filter((row) => row.description !== 'egresos diversos')
                .map((row, index) => (
                  <View key={`row-left-${index}`} style={styles.columnRow}>
                    <Text style={styles.columnRowLabel}>{row.description}</Text>
                    <Text style={styles.columnRowAmount}>{formatAmount(row.total, 'S/ ')}</Text>
                  </View>
                ))}

            {/* Total general solo para columna izquierda */}
            <View style={styles.totalRow}>
              <Text style={styles.totalLabel}>TOTAL GENERAL</Text>
              <Text style={styles.totalAmount}>{formatAmount(data.totalGeneral, 'S/ ')}</Text>
            </View>
          </View>
          ...
          <View style={styles.columnRight}>
            {Array.isArray(data?.rows) &&
              data.rows
                .filter((row) => row.description === 'egresos diversos')
                .map((row, index) => (
                  <View key={`row-right-${index}`} style={styles.columnRow}>
                    <Text style={styles.columnRowLabel}>{row.description}</Text>
                    <Text style={styles.columnRowAmount}>{formatAmount(row.total, 'S/ ')}</Text>
                  </View>
                ))}
